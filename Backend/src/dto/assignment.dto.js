...
const assignmentDto = {
	create: async (data, userId, transaction) => {
		...
		if (!member) throw new UnauthorizedError("Bạn không có quyền tạo bài kiểm tra này.");
	...
	update: async (id, data, userId, role, transaction) => {
		...
		if (!(role !== ROLE.ACADEMIC_AFFAIR && assignment.assignment_type === ASSIGNMENT_TYPE.EXAM) ||
