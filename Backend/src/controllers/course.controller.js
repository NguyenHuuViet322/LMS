...
import { COURSE_TIME_STATUS, ROLE, TAG_MODE, TAG_TYPE } from "../utils/const.js";
...
const courseController = {
	...
	update: async (req, res, next) => {
		...
		try {
			...
			course = await courseDto.update(course, req.body, transaction);
			if (req.body.tag_ids) {
				const tags = await tagDto.findByIds(req.body.tag_ids, transaction);
				const tagsValid = await courseDto.checkTags(req.body.tag_ids, transaction);
				if (!tagsValid) {
	...
	approve: async (req, res, next) => {
		...
		try {
			...
			await courseDto.update(course, { is_approved: true }, transaction);
			await transaction.commit();
	...
	assignRoom: async (req, res, next) => {
		...
		try {
			...
			if (canGenerate && courseShifts.length > 0) {
				let index = 1;
				for (let d = course.start_date; d <= course.end_date; d.setDate(d.getDate() + 1)) {
				...
				await courseDto.update(course, { is_generated: true }, transaction);
				await transaction.commit();
