...
const OverviewTab = ({ student }) => {
  return (
    <>
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">Thông tin học sinh</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
        ...
        <div className="flex items-center gap-3">
          <span className="text-lg">📞</span>
          <span><strong>Số điện thoại:</strong> {student.phone_number ?? "Chưa cập nhật"}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">🎂</span>
          <span><strong>Ngày sinh:</strong> {new Date(student.date_of_birth).toLocaleDateString()}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">🏠</span>
          <span><strong>Địa chỉ:</strong> {student.address ?? "Chưa cập nhật"}</span>
        </div>
        ...
        {student.additional_student_data && (
          <>
            <div className="flex items-center gap-3">
              <span className="text-lg">🏫</span>
              <span><strong>Trường:</strong> {student.additional_student_data.school ?? "Chưa cập nhật"}</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-lg">📚</span>
              <span><strong>Lớp:</strong> {student.additional_student_data.class ?? "Chưa cập nhật"}</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-lg">🆔</span>
              <span><strong>CCCD:</strong> {student.additional_student_data.cccd ?? "Chưa cập nhật"}</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-lg">👨‍👩‍👦</span>
              <span><strong>Người liên hệ:</strong> {student.additional_student_data.first_contact_name ?? "Chưa cập nhật"}</span>
            </div>
          </>
        )}
      </div>
...
const StudentProfilePage = ({ id }) => {
  ...
  const tabs = [
    { key: 'overview', title: 'Tổng quan', icon: <HomeIcon fontSize="small" />, component: <OverviewTab student={student} /> },
      ...
      <div className="relative w-full h-64 rounded-lg overflow-hidden mb-6">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 flex items-end">
          <div className="relative w-24 h-24 rounded-sm overflow-hidden border-2 border-white mr-4">
            <Image
              src={processGoogleDriveLink(student.avatar_url) || '/images/default_avatar.jpg'}
              layout="fill"
              objectFit="cover"
              alt="Avatar"
            />
