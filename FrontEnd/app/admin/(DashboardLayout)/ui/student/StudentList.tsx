...
const StudentList = () => {
  ...
  const { fileInputRef, handleClickImport, handleFileUpload, loading, formattedData } = useExcelUpload(() => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const student = response.data.filter(s => s.additional_student_data && (!filters.school || s.additional_student_data.school?.includes(filters.school)) && (!filters.class || s.additional_student_data.class?.includes(filters.class)));
  ...
  const downloadFileMau = async () => {
    const response = await fetch('/xlsx/File-mau.xlsx');
  ...
  return (
    <>
      <div className="rounded-lg dark:shadow-dark-md shadow-md bg-white dark:bg-darkgray p-6 relative w-full break-words">
                  ...
                  <Table.Row key={index}>
                    <Table.Cell className="whitespace-nowrap ps-6">
                      <div className="flex gap-3 items-center group relative">
                        <div className="truncat line-clamp-2 sm:text-wrap max-w-56 cursor-pointer" onClick={() => router.push(`/admin/ui/student/detail/${item.id}`)}>
                          <Badge color={'indigo'}>
                            {item.code}
                          </Badge>
                        </div>
        ...
        <StudentExcelModal studentData={formattedData} isOpen={isExcelImported} onClose={() => setIsExcelImported(false)} onConfirm={handleSubmitImport}></StudentExcelModal>
      </div>
    </>
  );
};
