...
const TimeTable = () => {
    ...
    const handleDayClick = (day: any) => {
        const dayLessons = lessons.filter(lesson => lesson.day.isSame(day, 'day'));
    ...
    const fetchData = async () => {
        try {
            ...
            const todayLessons = lessonsData.filter((lesson: any) => lesson.day.isSame(today, 'day'));
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DateCalendar
                ...
                slotProps={{
                    day: (ownerState) => {
                        const isHighlighted = lessons.some(
                            (lesson) => lesson.day.isSame(ownerState.day, 'day')
                        );
                ...
                {thisDayLesson === null ? (
                    ...
                    <div>
                        <Typography variant="body2" className="mb-3 p-3">
                            Bạn có {thisDayLesson.length} ca học trong ngày {currentMonth.format('DD/MM/YYYY')}
                        </Typography>
                                        ...
                                        if (lesson.code.substring(0, 2) === subject.code_fragment) {
                                            return (
                                                <div key={subject.id} className="flex items-center gap-2">
                                                    <Image src={subject.image} alt={subject.name} className="w-8 h-8" width={32} height={32} />
                                                    <div>
                                                        <div className="font-semibold">{lesson.name}</div>
                                                        <div className="text-sm text-gray-600">Mã khóa học: {lesson.code}</div>
                                                        <div className="text-sm text-gray-600">Ca học: {lesson.shift}</div>
                                                        <div className="text-sm text-gray-600">Phòng học: {lesson.room}</div>
                                                    </div>
                                                </div>
                                            )
                                        }
