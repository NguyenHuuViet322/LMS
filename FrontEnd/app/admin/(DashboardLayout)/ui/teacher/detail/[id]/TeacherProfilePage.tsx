  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
        ...
        <div className="flex items-center gap-3">
          <span className="text-lg">📞</span>
          <span><strong>Số điện thoại:</strong> {teacher.phone_number}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">🎂</span>
          <span>
            <strong>Ngày sinh:</strong> {new Date(teacher.date_of_birth).toLocaleDateString()}
          </span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">🏠</span>
          <span><strong>Địa chỉ:</strong> {teacher.address}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">🎓</span>
          <span><strong>Học hàm:</strong> {teacher.additional_teacher_data.portfolio.hoc_ham}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">📚</span>
          <span><strong>Môn giảng dạy:</strong> {teacher.additional_teacher_data.subject.name}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">🏫</span>
          <span><strong>Trường hiện tại:</strong> {teacher.additional_teacher_data.portfolio.school}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg">📅</span>
          <span><strong>Bắt đầu giảng dạy:</strong> {teacher.additional_teacher_data.portfolio.start_teaching_year}</span>
        </div>
      </div>
      <div className="mt-6 p-4 bg-gray-100 border-l-4 border-blue-500 rounded-md">
        <p className="text-gray-700 italic">&quot;{teacher.additional_teacher_data.portfolio.teaching_philosophy}&quot;</p>
      </div>
    </>
  );
};
...
const TeacherProfilePage = ({ id }) => {
  ...
  const tabs = [
    { key: 'overview', title: 'Tổng quan', icon: <HomeIcon fontSize="small" />, component: <OverviewTab teacher={teacher} /> },
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 flex items-end">
          ...
          <div className="text-white">
            ...
            <div className="flex items-center mt-2">
              ...
              <Badge color="success">
                {teacher.additional_teacher_data.portfolio?.hoc_ham || 'Giáo viên'}
              </Badge>
            </div>
          </div>
        </div>
