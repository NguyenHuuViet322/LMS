...
const TimeTableModal = ({ open, setClose }) => {
    ...
    return (
        <Modal show={open} onClose={setClose} size="7xl" className="rounded-lg shadow-xl">
                        ...
                        <Table hoverable className="table-fixed w-full">
                            <Table.Head className="bg-blue-100">
                                {headerConfig.map((header) => (
                                    <Table.HeadCell key={header.text} className={`text-center font-semibold ${header.width}`}>
                                        {header.text}
                                    </Table.HeadCell>
                                ))}
                            </Table.Head>
                            ...
                            <Table.Body className="divide-y divide-gray-300">
                                {[1, 2, 3, 4, 5, 6].map((shift) => (
                                    <Table.Row key={shift} className="hover:bg-gray-100 divide-x divide-gray-300">
                                        <Table.Cell className="text-center font-bold bg-gray-200 w-[80px]">{shiftName[shift]}</Table.Cell>
                                        {[2, 3, 4, 5, 6, 7, 1].map((day) => (
                                            <Table.Cell className="p-4 text-center w-[150px]" key={day}>
                                                {timeTableData[shift]?.[day] ? (
                                                    <div className="bg-blue-50 p-2 rounded-md shadow-md border border-blue-200">
                                                        <div className="font-semibold text-blue-600">{timeTableData[shift]?.[day]?.name}</div>
                                                        <div className="text-sm text-gray-600"><strong>Mã:</strong> {timeTableData[shift]?.[day]?.code ?? 'N/A'}</div>
                                                        {timeTableData[shift]?.[day]?.room && roomData[timeTableData[shift]?.[day]?.room] ? (
                                                            <div className="text-sm text-gray-600">
                                                                <strong>Phòng:</strong> Tầng {roomData[timeTableData[shift]?.[day]?.room].floor} -
                                                                Cơ sở {roomData[timeTableData[shift]?.[day]?.room].campus}
                                                                {roomData[timeTableData[shift]?.[day]?.room].room ? ` - Phòng ${roomData[timeTableData[shift]?.[day]?.room].room}` : ''}
                                                            </div>
                                                        ) : (
                                                            <div className="text-sm text-gray-600"><strong>Phòng:</strong> N/A</div>
                                                        )}
                                                        <div className="text-xs text-gray-500">{timeTableData[shift]?.[day]?.description ?? 'Chưa có mô tả'}</div>
                                                    </div>
                                                ) : (
