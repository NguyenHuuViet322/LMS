import { Badge, Button, Modal, Spinner, Table, TextInput } from "flowbite-react";
...
const AddMemberModal = (props: any) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const filterParams = Object.entries(filters)
                ...
                .join('&');
            const res = await callApi(`courses/${id}/not-members?filter[role]=STUDENT&${filterParams}`, "GET");
            setMembers(res);
        } catch (error) {
            console.log(error);
        } finally {
    ...
    const selectableMembers = useMemo(() => members.filter(member => member.status === null), [members]);

    // Check if all selectable members are selected
    const isAllSelected = useMemo(() =>
        selectableMembers.length > 0 && selectedMembers.size === selectableMembers.length,
        [selectableMembers, selectedMembers]
    );

    // Check if some selectable members are selected (for indeterminate state)
    const isIndeterminate = useMemo(() =>
        selectedMembers.size > 0 && selectedMembers.size < selectableMembers.length,
        [selectableMembers, selectedMembers]
    );
    ...
    const handleSelectAll = () => {
        if (isAllSelected) {
            ...
            setSelectedMembers(new Set(members.filter(member => member.status === null).map(member => member?.user?.id))); // Select only those with status === null
        }
    };
            ...
            <Modal.Body>
                <div>
                    <div className="flex gap-2">
                        <TextInput value={tempFilters.code} onChange={(e) => handleFilterChange('code', e.target.value)} className="w-1/4" placeholder="Mã học sinh" />
                        <TextInput value={tempFilters.name} onChange={(e) => handleFilterChange('name', e.target.value)} className="w-1/4" placeholder="Tên học sinh" />
                        <TextInput value={tempFilters.email} onChange={(e) => handleFilterChange('email', e.target.value)} className="w-1/4" placeholder="Email" />
                        <Select
                    ...
                    {isLoading ? <div className="flex justify-center items-center h-full">
                        ...
                        <Table>
                            <Table.Head className="sticky top-0 z-10 bg-white">
                                ...
                                <Table.HeadCell className="flex-grow text-md">Tên học sinh</Table.HeadCell>
