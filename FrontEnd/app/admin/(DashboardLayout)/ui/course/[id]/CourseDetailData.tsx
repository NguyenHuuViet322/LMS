...
import { Grade, gradeBadgeColors, RoleType, shiftDay, shiftName, Subject, subjectBadgeColors } from "@/app/utils/constant";
        ...
        <Card className="border-blue-200">
            <div className="flex items-center justify-between">
                ...
                {[RoleType.ADMIN, RoleType.TEACHER].includes(role) && (
                    <Button color="lightprimary" href={`/admin/ui/course/${course?.id}/edit`}><EditIcon />Sửa thông tin</Button>
                )}
            </div>
            ...
            <div className="flex items-center gap-2">
                <h1>Môn học: </h1>
                {subjectName && (
                    <Badge color={subjectBadgeColors[subjectName]}>
                        {subjectName}
                    </Badge>
                )}
                {gradeName && (
                    <Badge color={gradeBadgeColors[gradeName]}>
                        {gradeName}
                    </Badge>
                )}
            </div>
            <div>
                ...
                <div className="ml-4 mt-2">
                    {course?.shifts && course?.shifts.map((shift: any, index: number) => (
                        <div key={index} className="mb-1 flex items-center gap-2">
                            <span>{index + 1}.</span>
                            <span className="flex gap-1">
                                <Badge color="purple">{shiftDay[shift.day]}</Badge> -
                                <Badge color="pink">Ca {shiftName[shift.shift]}</Badge> -
                                <Badge color="indigo">{shift.room ? `Phòng ${shift.room}` : '- Chưa có phòng'}</Badge>
                            </span>
                        </div>
                    ))}
                </div>
