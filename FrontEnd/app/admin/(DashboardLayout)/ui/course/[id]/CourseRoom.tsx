...
import { Grade, gradeBadgeColors, RoleType, shiftDay, shiftName, statusBadge, Subject, subjectBadgeColors } from "@/app/utils/constant";
    ...
    return (
        <>
            {course.shifts.map((item, index) => (
                ...
                <div key={index} className="relative bg-white pb-3 pt-2 px-6 rounded-xl w-full my-4 shadow-xl border border-dashed border-primary transition-all duration-300 hover:-translate-y-2 hover:border-double">
                    <Tooltip title="Gán phòng học" placement="top">
                        <button onClick={() => handleAlterModalOpen(item.id, true)} type="button" className="absolute top-2 right-1 transition-all duration-200 text-blue-700 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-sm text-sm p-1.5 text-center inline-flex items-center me-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:focus:ring-blue-800 dark:hover:bg-blue-500">
                        ...
                        <div className="flex justify-between">
                            ...
                            <div className="my-2">
                                ...
                                <div className="flex space-x-2">
                                    {item.room && <Badge color={'success'}>Đã được gán phòng</Badge>}
                                    {!item.room && <Badge color={'warning'}>Cần được gán phòng</Badge>}
                                </div>
                                </div>
                            </div>
                        </div>
