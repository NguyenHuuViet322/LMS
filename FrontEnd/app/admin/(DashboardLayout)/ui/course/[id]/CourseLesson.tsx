...
const CourseLesson = ({course}) => {
    ...
    const handleAlterDate = async (data) => {
        ...
        try {
            ...
            notify((data.isRoom)?"Chuyển phòng học thành công":"Đổi lịch học thành công", 'success');
            await fetchLessonData();
        } catch (error) {
            console.error('Có lỗi', error);
            notify(error.message, 'error');
    ...
    return (
        <>
            <BlogSpinner isLoading={isLoading}/>
            {
                ((lesson.length == 0 && !isLoading) && (
                    <div className="w-full flex justify-center items-center mt-4">
                        <Image alt="Không thấy dữ liệu" src="/images/no_data.jpg" width="500" height="500"></Image>
                    </div>
                ))
            }
                    ...
                    lesson.map((item, index) => {
                        return (
                            <div key={index} className="relative bg-white pb-3 pt-2 px-6 rounded-xl w-full sm:w-1/2 max-w-md my-4 shadow-xl border border-dashed border-primary transition-all duration-300 hover:-translate-y-2 hover:border-double">
                                ...
                                <div className="mt-2">
                                    <p className="text-xl font-semibold my-2 text-blue-500">Thông tin ca học</p>
                                    ...
                                    <div className="flex flex-wrap gap-2 text-gray-400 text-sm my-3">
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            <Badge color="purple">{formattedDate(item.take_place_at)}</Badge>
            ...
            <ConfirmModal
                ...
                type={"default"}            
            />
            <LessonAlterModal onClose={handleAlterModalClose} openModal={isAlterModalOpen} onSubmit={handleAlterDate} isRoom={isRoom}></LessonAlterModal>
        </>
    )
}
