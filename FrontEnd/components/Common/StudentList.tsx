...
const FeaturedTeachers = () => {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      const response = await callApi('user?limit=6&filter[role]=TEACHER&sort[date_of_birth]=ASC');
      setTeachers(response.data.filter((teacher: any) => teacher.additional_teacher_data));
    };
    fetchTeachers();
  }, []);
  ...
  return (
    <section className="dark:bg-bg-color-dark bg-gray-light relative z-10 py-16 md:py-20 lg:py-28">
      <SectionTitle
        ...
        paragraph="Đội ngũ giáo viên có kinh nghiệm và chất lượng cao chính là cơ sở để HAPPY EDU HUB có thể phát triển và mang đến cho học viên những trải nghiệm tuyệt vời nhất."
        center
      />
      ...
      <div className="flex items-center justify-center gap-5 h-96 w-1/2 flex-nowrap mx-auto -mt-12">
        <Carousel theme={{
          ...
          {teachers.map((teacher) => (
            <div
              key={teacher.id}
              className="w-[380px] h-80 bg-white shadow-two rounded-sm flex flex-shrink-0 transition-all duration-300 transform hover:shadow-lg hover:scale-105 ease-in-out"
            >
              <div className="w-1/3">
                <img
              ...
              <div className="w-2/3 p-4 flex flex-col">
                ...
                <div className="mt-4 text-sm text-gray-900 font-semibold">
                  GV {teacher.additional_teacher_data.subject.name} - {teacher.additional_teacher_data.portfolio.school}
                </div>
              </div>
            </div>
          ))}
